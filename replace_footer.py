...
root_new_footer = new_footer.replace('href="', 'href="pages/').replace('pages/../', '').replace('pages/#', '#')

def process_file(filepath, is_root=False):
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Use regex to find <footer class="footer">...</footer>
    pattern = re.compile(r'<footer class="footer.*?</footer>', re.DOTALL)
    
    if not pattern.search(content):
        return
        
    replacement = root_new_footer if is_root else new_footer
...
if os.path.exists('index.html'):
    process_file('index.html', is_root=True)

if os.path.exists('pages'):
    for filename in os.listdir('pages'):
        if filename.endswith('.html'):
            process_file(os.path.join('pages', filename), is_root=False)
